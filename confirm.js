...
Elm.Confirm = Elm.Confirm || {};
Elm.Confirm.make = function (_elm) {
   "use strict";
   _elm.Confirm = _elm.Confirm || {};
   if (_elm.Confirm.values)
   return _elm.Confirm.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   model) {
      return function () {
         ...
         {case "Add":
            ...
            case "Prompt": return model;}
         _U.badCase($moduleName,
         "between lines 41 and 47");
      }();
   });
   var addPlayer = Elm.Native.Port.make(_elm).inboundSignal("addPlayer",
   "String",
   function (v) {
   ...
   var externalActions = $Signal.mergeMany(_L.fromArray([A2($Signal._op["<~"],
   Add,
   addPlayer)]));
   ...
   model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([$Html.text(model.name)
                   ,A2($Html.button,
                   _L.fromArray([A2($Html$Events.onClick,
                   address,
                   Prompt)]),
                   _L.fromArray([$Html.text("Set Name")]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var model = function () {
      var allActions = $Signal.mergeMany(_L.fromArray([actions.signal
                                                      ,externalActions]));
   ...
   model);
   var displayPrompt = Elm.Native.Port.make(_elm).outboundSignal("displayPrompt",
   function (v) {
      return [];
   },
   $Signal.map($Basics.always({ctor: "_Tuple0"}))(A2($Signal.filter,
   function (s) {
      return _U.eq(s,Prompt);
   },
   NoOp)(actions.signal)));
   _elm.Confirm.values = {_op: _op
                         ,NoOp: NoOp
                         ,Prompt: Prompt
                         ,Add: Add
                         ,actions: actions
